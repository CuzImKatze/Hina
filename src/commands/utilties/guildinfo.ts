...
module.exports.run = async (prefix, cmd, client, args, message) => {
    ...
    .addField(await client.string(message.guild.id, "userinfo.roles"), message.guild.roles.cache.sort(compare).map(roles => roles).join(", ").substr(0, 1000))
    .addField(await client.string(message.guild.id, "userinfo.created"), moment(message.guild.createdAt).format("dddd, Do MMMM YYYY, HH:mm:ss"))
